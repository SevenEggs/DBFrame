...
void SakilaDatabaseConnection::DoPrepareStatements()
{
    ...
    PrepareStatement(SAKILA_SEL_ACTOR_INFO, 
    "select actor_id, first_name, last_name, last_update from actor where actor_id = ?", CONNECTION_SYNCH);
    PrepareStatement(SAKILA_SEL_ACTOR_INFO_ASYNC, 
    "select actor_id, first_name, last_name, last_update from actor where actor_id = ?", CONNECTION_ASYNC);
}
