...
DatabaseLoader& DatabaseLoader::AddDatabase(DatabaseWorkerPool<T>& pool, 
                                            ...
                                            const uint8 asyncThreads,
                                            const uint8 synchThreads)
{
    // bool const updatesEnabledForThis = DBUpdater<T>::IsEnabled(_updateFlags);

    _open.push([this, dbstring, &pool, asyncThreads, synchThreads]() -> bool
    {
        pool.SetConnectionInfo(dbstring , 
                                asyncThreads, synchThreads);
        if (uint32 error = pool.Open())
        {
            ...
            if (error)
            {
                TC_LOG_ERROR("sql.driver", "\nDatabasePool {} NOT opened. There were errors opening the MySQL connections. Check your SQLDriverLogFile "
                    "for specific errors. Read wiki at http://www.trinitycore.info/display/tc/TrinityCore+Home", dbstring);
    ...
    _prepare.push([this, dbstring, &pool]() -> bool
    {
        ...
        {
            TC_LOG_ERROR(_logger, "Could not prepare statements of the {} database, see log for details.", dbstring);
            return false;
        }
        return true;
    });
